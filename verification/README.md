...
| 4 | **Model actually generates a response** | The one the manual checklist can't do at fleet scale. Records latency |
...
| 10 | Example `.lsp` files parse | An example that won't `APPLOAD` wastes 80 people's first ten minutes |

Check 4 is the slow one. `-SkipGeneration` skips it when you're re-testing
something else, at the cost of a WARN recording that the model was never
exercised.
